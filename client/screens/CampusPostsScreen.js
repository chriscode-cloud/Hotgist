...
const CampusPostsScreen = () => {
  ...
  const { campusId, campusName } = route.params;
  ...
  const fetchPosts = async () => {
    try {
      setLoading(true);
      // Fetch posts from specific campus
      const response = await apiService.getPosts(campusId, 50, 0, false);

      if (response.success) {
        setPosts(response.posts);
        // Get campus info if not already set
        if (!campusInfo && response.posts.length > 0) {
          setCampusInfo({
            id: campusId,
            name: campusName || response.posts[0].campusName || campusId
          });
        }
      } else {
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyStateSubtitle}>
        Be the first to share something with the {campusName || campusId} community!
      </Text>
      <TouchableOpacity
    ...
    <View style={styles.container}>
      ...
      <View style={styles.campusHeader}>
        <View style={styles.campusInfo}>
          ...
          <Text style={styles.campusTitle}>{campusName || campusId}</Text>
        </View>
        <Text style={styles.campusSubtitle}>
          🏫 Posts from this campus community
        </Text>
      </View>
...
export default CampusPostsScreen;